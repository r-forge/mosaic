mCoverage = function(expr, data){
 ...
 myFun=function(n=n, conf.level=0.95,xpick=xpick,ntrials=10,seed=125, int.type=Wald){
  set.seed(seed)
...
#==========
manipulate(myFun(n=n, conf.level=conf.level,xpick=xpick, ntrials=ntrials, seed=seed, int.type=int.type),
           n = slider(5, 500, step = 1, initial=100, label = "Sample Size"),
           xpick=picker(xvar, label="Dep. Variable to plot"),
