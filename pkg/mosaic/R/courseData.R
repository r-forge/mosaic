# ============== Local Data Storage ========
.courseData.storage.helper = function( ){
   local.library = list()
   ...
   fun = function(library=FALSE,searchpath=FALSE,val=NULL,name=NULL,action) {
     ...
     stop("Can use only for the courseData library and search path.")
   }
   return(fun)
}
# ===============
.courseData.storage =.courseData.storage.helper() # run the function to create .courseData.storage

# ============
courseData = function(name=NULL,
  add.to.path=FALSE, drop.from.path=FALSE,
  add.to.library=FALSE, directory=NULL, var=NULL){
  # ==== load a data set to the local library
  if (add.to.library) {
      if( !is.null(var)) {
        .courseData.storage(name=name,val=var,library=TRUE,action="add")
      }
      else {
        goo = courseData(name)  # get it from the web site
        if (exists("goo") ) .courseData.storage(name=name,val=goo,library=TRUE,action="add") 
  ...
  if (add.to.path) {
    if (!is.null(name) )
      .courseData.storage(searchpath=TRUE,name=name,action="add")
    return(.courseData.storage(searchpath=TRUE,action="get"))
  }
  if (drop.from.path) { # leaving out name means that path will be emptied.
      return(.courseData.storage(searchpath=TRUE,name=name,action="delete"))
  }
  # ==== Look for file in a local directory.
  if( is.null(name) )
    return( read.csv( file.choose() ))
  # ==== Look on web, in library, in packages.         
  if (name %in% .courseData.storage(library=TRUE,action="names") )
    return( .courseData.storage(library=TRUE,action="get",name=name) )
  else {
    res = NULL
    # look for it in the web sites
    web.sites = c(directory,.courseData.storage(searchpath=TRUE,action="get"))
    for (k in web.sites) {
