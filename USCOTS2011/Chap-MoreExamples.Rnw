...
The \verb!mosaic! package contains three forms of the de-identified HELP dataset:
\begin{description}
\item[HELP] a minimal dataset containing XX variables for the XX subjects
with minimal missing data, primarily at baseline
\item[HELPbig] a larger dataset including followup information and more variables
\item[HELPhuge] an even larger dataset with all variables for all subjects
at all timepoints.
\end{description}
...
{\tt anysubstatus} & use of any substance post-detox (0=no, 1=yes)  & see also {\tt daysanysub}
\\ \hline
...
{\tt sexrisk} & Risk-Assessment Battery (RAB) drug risk score  (range 0--21)  & see also {\tt drugrisk}
\\ \hline
{\tt substance} & primary substance of abuse (alcohol, cocaine or heroin) &
\\ \hline
{\tt treat} & randomization group (0=usual care, 1=HELP clinic)  &
\\ \hline
\end{longtable}
\noindent
