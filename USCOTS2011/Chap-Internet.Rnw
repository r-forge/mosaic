...
Google Docs allows students and instructors to create various forms of
...
Once on the Google Docs system, the documents can be edited 
...
\begin{itemize}
  \item Students working on group project can all simultaneously have
...
An effective technique for organizing the student work and ensuring
...
Then set the permission on this directory to share it with the
student.  Anything he or she drops into the directory is automatically
...
collect data to inform whether the ``close door'' button on an
...
Once you have a spreadsheet file in Google Docs, you will want to open
...
\paragraph{If you are using other interfaces for \R}, you must first use the Google
...
It turns out that communicating with Google Docs requires facilities
...
into a data frame:
<<>>=
elev = readGoogleCSV("https://spreadsheets.google.com/spreadsheet/pub?hl=en&hl=en&key=0Am13enSalO74dEVzMGJSMU5TbTc2eWlWakppQlpjcGc&single=true&gid=0&output=csv")
head(elev)
@ 
...
As an alternative to the code above, we could first build a dataframe and then use that for our 
plot.
<<fig=false>>=
...
@
\authNote{perhaps use reshape as another method?}%
...
\function{userFollowers} & returns lists of class {\tt user} \\
\function{searchTwitter} & returns lists of class {\tt status} \\
\function{Rtweets} & hardcodes a search for {\tt \#rstats} \\
...
<<twitter2>>=
censustweets <- userTimeline(census, n=5)
censustweets
@
...
<<twitter2b>>=
censustweets[[1]]$getText()
censustweets[[1]]$getCreated()
censustweets[[1]]$getId()
@
\InstructorNote{section \ref{sec:datastruct} provides details on accessing lists}
...
<<twitter2c>>=
censusfollow <- userFollowers(census)
count <- sapply(censusfollow, function(x) x$getStatusesCount())
favstats(count)
censusfollow[[which.max(count)]]
@
\InstructorNote{the \function{sapply()} applies a function to each element of a list}
