...
\function{getUser(userstring, ...)} & returns an object of class {\tt user} \\
\function{userFriends(user, ...)} & returns lists of class {\tt user} \\
\function{userFollowers(user, ...)} & returns lists of class {\tt user} \\
\function{searchTwitter(searchstring, ...)} & returns lists of class {\tt status} \\
\function{Rtweets(...)} & hardcodes a search for {\tt \#rstats} \\
\function{showstatus(id, ...)} & take a numeric ID of a tweet and return it \\
\function{publicTimeline(...)} & current snapshot of the public timeline \\
\function{userTimeline(user, ...)} & current snapshot of the users public timeline \\ \hline
\end{tabular}
\end{center}
\end{table}
...
census$getCreated()
@
Detailed reports of the last {\tt n} twitter status updates can be returned.
<<twitter2>>=
censustweets <- userTimeline(census, n=10)
censustweets
