...
\centerline{\includegraphics[width=3.8in]{images/zillow_api.pdf}}

\centerline{\includegraphics[width=3.8in]{images/zillow_profile.pdf}}

The first step is to install the Zillow package, which unlike others described
to date, is available on a different repository than CRAN:
...
@
Before you proceed, you need to set your \verb!zillowID! to the value that you 
got when you registered with {\tt Zillow.com}.
<<zillow1>>=
est = "Zillow package not installed"
comps = est
if(require(Zillow)) {
  est = zestimate("1538 SE 33rd Avenue", "97214", zillowId)
