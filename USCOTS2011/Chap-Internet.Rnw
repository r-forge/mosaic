...
from the Internet.   We draw your attention particularly to 
...
This technique makes it straightforward to distribute data with little
...
use the appropriate \R\ command: \texttt{load( )} for a workspace or
\texttt{source( )} for a script.
...
\texttt{source( )} command to re-read the script.
...
you (and/or your students) generate a \options{zillowId} for use with the system.
...
the next step is to install the \verb!Zillow! package. The \verb!Zillow! package is
...
Next, you should initialize your \verb!zillowID! to the value that you 
...
(tax) value.
\authNote{NH: that's a better plot, but not the simplest syntax I've ever seen...}
