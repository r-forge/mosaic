...
\Caution{Note that the comparison operator is \emph{two} equal signs}
<<cesd-stem>>=
...
<<subsmale,fig=TRUE>>=
male <- subset(HELP, sex=="male")
bwplot(cesd ~ substance, data=male)
@
\end{solution}
...
\authNote{NH: is there a way to turn off the rug plot (though by default
...
the \VN{results} object.  For example, the user can extract the confidence interval.
<<binomtest2>>=
results$conf.int
results$conf.int[1]
@
...
@
\end{center}
\authNote{NH: how to add a superimposed normal density?}
...
\authNote{NH: I'd welcome suggestions about how to format this?}
...
The \function{bwplot()} function can be used to display the boxplots for the CESD scores separately by sex.  We see from both the numerical and graphical
displays that women tend to have slightly higher CESD scores than men.
\authNote{Should mention use of \function{xyplot()} when the sample sizes are small.
...
dotPlot(~ abs(diff), rtest.stats, n=40, 
