...
\vspace{-8mm}
\begin{center}
<<cesd-dot,fig=TRUE,width=3,height=1.6, eval=FALSE>>=
dotPlot(~cesd, data=female)
@
\end{center}
We could also have made our subset ``on the fly'', just for the purposes of graphing:
<<cesd-dot2,fig=TRUE,width=3,height=1.6,eval=FALSE>>=
dotPlot(~cesd, data=HELP, subset=female==1)
@
Or we could make side-by-side dotplots of men and women:
\begin{center}
<<cesd-dot3,fig=TRUE,width=5,height=1.9,eval=FALSE>>=
dotPlot(~cesd | female, data=HELP)
@
\end{center}

Nicer if we recode:
\begin{center}
<<cesd-dot4,fig=TRUE,width=5,height=1.9,eval=FALSE>>=
...
\vspace{-8mm}
\begin{center}
<<cesd-box,fig=TRUE,width=3,height=1.6,eval=FALSE>>=
