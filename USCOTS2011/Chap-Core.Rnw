...
class(model)
@
The return value from \verb!lm()! is a linear model object;
A number of functions can operate on these objects, as
seen previously with \verb!coef()!.  As an additional example,
there is a function called \verb!residuals()! that will return a
vector of
the residuals.
...
The \verb!xchisq.test()! function provides additional details to help with this process.
...
The \verb!aggregate()! function can be used to calculate the mean CESD score
...
The \verb!anova()! command can be used to compare two (nested) models.
...
Logistic regression is available from within the \verb!glm()! function: a variety of
