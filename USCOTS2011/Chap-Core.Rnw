...
\FoodForThought{Density plots are also sensitive to certain choices.  If you density plot
...
\authNoted{NH: is there a way to turn off the rug plot (though by default
I like having this here)}%
\authNote{rjp:  It's not really a rug, but in any case I don't know if there is a way to 
turn it off.  Seems like there should be, but I didn't see it in the documentation.}%
\authNote{rjp: added superimposed density plots as per Nick's wish.}%
...
\begin{center}
<<norm1,fig=TRUE,width=5,height=2.4>>=
xpnorm(1.96, 0, 1)
@
\end{center}
...
have done this already, but I don't know of a function to do it.  I've thought about writing one.}
...
<<>>=
binom.test(244, 209 + 244)
@
The \pkg{mosaic} package provides a formula interface that avoids the need to pre-tally
the data.
<<binomtest>>=
result <- binom.test(~ homeless=="housed", HELP)
result
names(result)
@

\authNoted{NH: this is pretty strange (I would have thought that this would have estimated
p=0.539).  rjp:  Was strange.  I've fixed this.  Now you get the value you wanted.}
...
<<>>=
prop.test(244, 209 + 244, correct=FALSE)
@
or counts of successes and failures as a single vector.
<<>>=
prop.test(c(244, 209), correct=FALSE)
@
...
\authNoted{NH: suggestions for a better approach?  rjp:  I've added prop() to mosaic.}
...
@
\end{center}
\authNoted{rjp: done. NH: how to add a superimposed normal density?}%
\begin{center}
<<HELP-resid-qq,fig=TRUE>>=
qqmath(~ resid(model))
@
\end{center}
\begin{center}
<<HELP-resid-plot,fig=TRUE>>=
...
\begin{multicols}{2}
\begin{center}
<<HELP-lmplot1,fig=TRUE,width=3,height=4>>=
plot(model, which=1)
@
<<HELP-lmplot2,fig=TRUE,width=3,height=4>>=
plot(model, which=2)
@
\end{center}
\end{multicols}
...
\authNoted{Should mention use of \function{xyplot()} when the sample sizes are small.
No reason to compute 5-number summaries of 10 or fewer numbers really.  NH resp: I'm not sure
what you mean by this. rjp: done}
\authNote{A slightly smaller data set might have been better, but this works.}%
...
\begin{center}
<<KidsFeet-xy,fig=true,height=1.5>>=
...
<<help-logit>>=
summary(glm(homeless ~ age + female, family=binomial, data=HELP))
@
...
<<help-aovplot,fig=TRUE>>=
bwplot(cesd ~ homeless | sex, data=HELP)
@
<<help-aov2>>=
...
summary(aov(cesd ~ homeless * sex, data=HELP))
@
There's little evidence for the interaction, though there is a statistically
significant main effects terms for \VN{sex} and a borderline significant
effect for \VN{homeless} status.

<<help-interaction,fig=true>>=
xyplot(cesd ~ homeless, data=HELP, groups=sex, type='a')
@
