...
\authNote{NH: my plan is to move this later in the section, with reference to Simulation.
I also plan to recast using do() if possible}
...
<<gof>>=
table(HELP$substance)
perctable(HELP$substance)
chisq.test(table(HELP$substance), p=c(1/3, 1/3, 1/3))
@
...
<<help-fisher>>=
with(HELP, fisher.test(homeless, female))
@
