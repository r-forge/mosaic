...
that the four numbers are equally likely.
...
(See samples \Sexpr{ paste(which( apply(rgolfballs, 1, max) >= 138 ), sep=', ') }.)
...
First, we generate a larger sample assuming each number is equally likely.
<<>>=
...
Our test statistic (the maximum count) feels like it isn't making full use of our data.  
...
test statistics computed on random samples from Plant Null.
\item
