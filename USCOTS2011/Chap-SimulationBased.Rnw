...
\authNote{DTK: The statements below using HELP refer to a variable
  named ``sex.''  There doesn't seem to be one at present.  I
  generated one from ``female'' so that I could compile the document.}
  
<<making-female-to-sex>>=
# Generate the variable "sex" from "female"
# FIX THIS!
HELP$sex = as.factor( c("M","F")[1+HELP$female] ) 
@ 
<<help102>>=
...
\authNote{Change the following to a ``data'' statement if we add
  ten-mile-race.csv to the mosaic package.}
<<>>=
our.population = read.csv("http://www.macalester.edu/~kaplan/ISM/datasets/ten-mile-race.csv")
nrow(our.population)
@ 
...
<<run-sample>>=
mysamp = sample(our.population, 100)
@ 
...
<<>>=
with(sample(our.population,100), mean(age))
with(sample(our.population,100), sd(age))
with(sample(our.population,100), mean(sex=="F"))
lm(net ~ age + sex, data=sample(our.population,100))
@ 
...
<<run-samp-dist>>=
s1 = do(500)*with(sample(our.population,100), mean(age))
s2 = do(500)*with(sample(our.population,100), sd(age))
s3 = do(500)*with(sample(our.population,100), mean(sex=="F"))
s4 = do(500)*lm(net ~ age + sex, data=sample(our.population,100))
@ 
...
<<>>=
mysamp = sample(our.population, 100) # a sample from the population
@ 
...
Examples: IN NOTE FORM FOR NOW.
\begin{itemize}
\item differences between two means
\item differences between two proportions
\item model coefficients.
\end{itemize}

Show single examples, then use ``do'' to construct the sampling distribution.
