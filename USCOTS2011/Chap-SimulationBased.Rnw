...
@
With a empirical p-value of approximately \Sexpr{round(mean(rtest.stats >= test.stat),3)}, there 
...
xhistogram(~diff, rtest.stats, v=test.stat, groups=diff >= test.stat)      
@
\end{center}
...
dotPlot(~diff, rtest.stats, n=40, groups=diff >= test.stat, pch=16 )
