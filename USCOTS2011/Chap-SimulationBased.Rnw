...
name of a family of distributions.  These functions all work similarly to produce 
...
\FoodForThought[\centerline{Tip}]{As students come up with test statistics, let them
...
empirical p-values with goal of providing a flexible tool 
...
\texttt{planet.sample} with a command to sample anew from the population.
...
\Caution{Remember to \function{resample()} rather than 
