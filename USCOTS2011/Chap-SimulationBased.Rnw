...
parameter.''  
\authNote{RJP: Why use scare quotes here?  Use \texttt{\term{}} to highlight terms.  We 
...
What makes \verb!do()! clever is that is knows about several types of things
we might like to repeat and it tries to keep track of the most important summary 
statistics for us.  
If we fit a linear model, 
for example, \verb!do()!  keeps track of the regression coefficients, 
$\hat \sigma$, and $r^2$.
<<>>=
...
Allan Rossman once lived along a golf course.  One summer he collect the golf balls that
landed in his yard and tallied how many were labeled with 1's, 2's, 3's, and 4's 
because he was curios to know whether these numbers were equally likely.%
...
\end{tabular}
\end{center}
We can enter this data into \R\ using the \verb!c()! function.
<<>>=
...
with thinking about how do create it.}%
...
\begin{center}
<<golfballs-max,fig=true,height=1.7>>=
...
Usually I get lucky and someone will suggest the worlds worst test statistic:
...
\begin{center}
<<golfballs-worst,fig=true,height=1.7>>=
print(statTally(golfballs, rgolfballs, function(x) { sum(x-121.5)} ))
@
\end{center}
...
print(statTally(golfballs, rgolfballs, function(x) { sum(abs(x-121.5)) }))
@
\end{center}
...
test statistics computed on random samples from Planet Null.
\item
...
function so that the following is equivalent:
<<>>=
...
and count the brown ones XX CITATION XX, they have students walk randomly to see how
far they get after $n$ steps XX CITATION XX, they toss a globe around the classroom
...
Earth in some ways and not in others.  
...
These hypothetical planets are:
...
\begin{enumerate}
  \item Make sure you can replicate the calculations for the standard
    error of one of the statistics in the \verb!TenMileRace! example with a
...
All the data for Planet Sample is in our computer, so \emph{although sampling
...
\verb!planet.sample! is the result of laborious data collection.

\authNote{R' deleted (at least temporarily):
...
replacement from Planet Sample \term{resampling} for short.  The \verb!resample()! function
in the \verb!\mosaic! package can do this as easily as sampling without replacement.
