...
@
\begin{center}
<<t-test-simulatoin-histogram,fig=true,width=5>>=
...
NOTES: This takes place on Planet Earth.  It requires the actual
population.  Example using the running data.

\section{The Re-Sampling Distribution}

NOTES: It's impractical to take repeated samples from Earth.  So we
take one sample and use it to populate Planet Sample.  Then we can use
the computer to  drawing many samples from Planet Sample.  This gives
us some information about how one sample is likely to differ from
another, but all the samples we draw on Planet Sample are necessarily
going to resemble sample with which we originally populated the
planet. 
