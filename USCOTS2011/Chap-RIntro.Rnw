...
example(histogram)
@
will generate a number of example plots (and you the commands used to create them).
...
\begin{center}
<<demo,eval=false,fig=T,width=2.2,height=1.8>>=
demo(funplots)
@
\end{center}
...
<<iris-dotPlot-condA,cache=T,eval=F,fig=F,width=6.0,height=2.5>>=
dotPlot(~Sepal.Length|Species, data=iris, n=20)
@
\begin{center}
<<iris-dotPlot-condB,cache=T,fig=T,echo=F,width=6.0,height=2.0>>=
...
\begin{center}
<<barchart1a,fig=T,width=3,height=1.8,echo=T>>=
barchart(xtabs(~sector, CPS))
@
<<barchart2a,fig=T,width=4,height=1.4,echo=T>>=
...
\begin{center}
<<mosaic1,fig=true,width=4,height=2>>=
...
\begin{center}
<<barchart3b,fig=T,echo=false,width=5>>=
