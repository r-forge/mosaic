...
\FoodForThought[\centerline{Tip}]{It's probably best to settle on using 
...
\end{itemize}
There may be others that we use from time to time as well.  
...
One a package is installed, it is available to be loaded in the current or any future session.
...
\authNote{R': Can't move \verb!findData()! into \verb!mosaic! since it is only for finding data in 
\verb!abd! and is designed for use with a text book to help figures out data frame names to match
...
\begin{itemize}
\item
\verb!Hmisc! (Frank Harell's miscellaneous utilities),
\item
...
\Caution{Avoid the use of \texttt{attach()}.}
...
variables using the \verb!\$! syntax or to use the \verb!with()! function.
...
\verb!read.csv()! or \verb!read.table()! (for white space delimited files).
The \verb!mosaic! package includes a function called \verb!read.file()! that uses 
slightly different default settings and infers whether it should use \verb!read.csv()!,
\verb!read.table!, or \verb!load()! based on the file name.  
...
@
\authNote{rjp: use \texttt{read.file} after fixing bug?}
...
The \verb!findData()! function makes in \verb!abd! it easy to map examples and exercises in that book to 
...
@
(Note: This requires installation of the \verb!Hmsic! package.)
