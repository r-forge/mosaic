...
\begin{center}
\emph{This is a lightly modified version of a handout Randy used with his 
...
\verb!Hmisc!, \verb!vcd!, \verb!fastR!, and \verb!abd! (in that order) the first time you use \R\ 
...
For a histogram, the \verb!y! component of the formula is empty since we let \R\
...
Here is a bad way to make side-by-side boxplots.
...
<<themes-fastR,cache=T,fig=T,height=4>>=
trellis.par.set(theme=col.fastR())        # a theme in the fastR package
show.settings()
@

<<themes-fastRbw,cache=T,fig=T,height=4>>=
trellis.par.set(theme=col.fastR(bw=TRUE)) # black and white version of previous theme
show.settings()
@
