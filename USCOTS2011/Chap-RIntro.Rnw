...
\R\ is a system for statistical computation and graphics.  We will use \R\ 
for several reasons:
\begin{enumerate}
\item \R\ is open-source and freely available for Mac, PC, and Linux machines.  This means that there is no restriction on having to license a particular software program, or have
...
advantages, in that it simplifies installation of R, allows work to begin on one machine, and be picked up seamlessly later on somewhere else.
...
<<function-syntax,eval=F>>=
functionname( argument1, argument2, ... )
@
\vspace{-5mm}
\FoodForThought[\centerline{Tip}]{To help students get the hang of function arguments,
...
\Caution{Your students will get into a syntactic hole from which they cannot
...
\authNote{Do we still need these other packages?}
\verb!Hmisc! and \verb!mosaic! 
the first time you use \R\ 
so that they are always available to you on that machine.
...
\authNote{Need to move findData into mosaic}
...
\begin{problem}
\authNote{Can we tweak this in some manner?}
...
The \verb!attach()! function in R can be used to make objects within dataframes accessible in R with fewer keystrokes, but we strongly discourage its use, as it often leads to name conflicts.
The Google R Style Guide (\url{http://google-styleguide.googlecode.com/svn/trunk/google-r-style.html}) echoes this advice, stating that \emph{The possibilities for creating errors when using attach are numerous. Avoid it.} It is far better to directly access variables using the \verb!\$! syntax
or to use the \verb!with()! function.
...
<<formula-interface, eval=F, echo=T>>=
...
\begin{center}
<<iris-dotPlot,cache=T,fig=T,width=5,height=3.0>>=
...
<<iris-dotPlot-condA,cache=T,eval=F,fig=F,width=7.0,height=2.5>>=
dotPlot(~Sepal.Length|Species, data=iris, n=20)
@
\begin{center}
<<iris-dotPlot-condB,cache=T,fig=T,echo=F,width=7.0,height=2.0>>=
...
\begin{center}
<<iris-histogram,cache=T,fig=T,width=3,height=1.8>>=
...
\begin{center}
<<iris-histogram-cond,cache=T,fig=T,width=4.5,height=2.0>>=
...
\begin{center}
<<iris-bwplot,fig=T,width=3,height=1.3>>=
...
\begin{center}
<<iris-bwplot-cond,cache=T,fig=T,width=5,height=1.6,print=T>>=
...
\begin{center}
<<iris-bwplot-2d,cache=T,fig=T,width=2,height=1.5,print=T>>=
bwplot(Sepal.Length~Species, data=iris)
@
\end{center}
...
\begin{center}
<<iris-bwplot-2dh,cache=T,fig=T,width=4,height=1.7,print=T>>=
...
\begin{center}
<<iris-xyplot,cache=T,fig=T,width=2.4,height=1.6,print=T>>=
...
\begin{center}
<<iris-xyplot-cond,cache=T,fig=T,width=4.5,height=1.7,print=T>>=
...
\begin{center}
<<iris-xyplot-groups,cache=T,fig=T,width=2.5,height=1.9,print=T>>=
...
\begin{center}
<<iris-xyplot-key,cache=T,fig=T,width=2.6,height=2.4,print=T>>=
xyplot(Sepal.Length ~ Sepal.Width, groups=Species, data=iris, 
	auto.key=TRUE)   
@
\end{center}
...
\begin{center}
<<iris-xyplot-alpha,cache=T,fig=T,width=2.7,height=2.0,print=T>>=
xyplot(Sepal.Length ~ Sepal.Width, groups=Species, data=iris, 
...
\begin{center}
<<iris-xyplot-text,cache=T,fig=T,width=3,height=2.3,print=T>>=
xyplot(Sepal.Length ~ Sepal.Width, groups=Species, data=iris, 
	main="Some Iris Data",
...
\begin{center}
<<barchart1a,fig=T,width=3,height=1.4,echo=T>>=
barchart(xtabs(~infection.status, Trematodes))
@
<<barchart2a,fig=T,width=3,height=1.4,echo=T>>=
...
<<TeenDeaths-1,fig=F,width=5,height=2.5,echo=T>>=
barchart(cause~deaths,TeenDeaths,horizontal=TRUE)
@
\begin{figure}
\begin{center}
<<TeenDeaths-2,fig=T,width=5,height=2.5,echo=F>>=
barchart(cause~deaths,TeenDeaths,horizontal=TRUE)
@
\end{center}
\vspace{-8mm}
...
\iffalse
\begin{center}
<<pie1,fig=T,width=3,height=3.0,echo=F>>=
pie(table(Trematodes$infection.status))
@
<<pie2,fig=T,width=3,height=3.0,echo=F>>=
pie(table(Trematodes$eaten))
@
\end{center}
...
<<mosaic1,fig=F,width=9,height=7.0,echo=T>>=
...
\begin{center}
<<mosaic2,fig=F,width=6,height=2.5>>=
...
\begin{center}
<<mosaic3,fig=T,width=6,height=2.5,eval=F>>=
mosaic(mycrosstable)
@
\end{center}
...
\begin{center}
<<mosaic4,fig=F,width=6,height=2.5>>=
...
\begin{center}
<<barchart3,fig=T,width=3,height=1.3,echo=F>>=
print( lattice::barchart( Freq ~ infection.status, groups=eaten, data=trem) )
@
\end{center}
...
\begin{center}
<<demo-sec2-2,fig=T,width=2.2,height=1.8>>=
demo(sec2.2)
@
\end{center}
\vspace{-8mm}
...
\subsection{Text and Quotations}
...
\authNote{NH: Are you both comfortable with this?}
