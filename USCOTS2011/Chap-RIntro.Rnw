...
\FoodForThought{Not all authors are equally skilled at creating examples.  Some of the examples
are next to useless, others are excellent.}
...
\function{mosaic()} (from the \pkg{vcd} package) is not a \verb!lattice! plot, 
