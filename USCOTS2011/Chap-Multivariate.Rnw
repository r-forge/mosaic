...
call ``phenotype,'' the observable characteristics themselves.  The MOSAIC
dataset \texttt{Galton} contains, transcribed to a modern format, measurements that Galton himself
...
\begin{problem}[stars=1]Consider the \texttt{CPS} data that gives per-hour wages and
...
\authNote{DTK: This chapter is currently created in
  Chap-Multivariate.Rnw, but should be moved to a separate Rnw file
  and included directly in the Master file.  I will wait
  until it's bigger and I have checked to make sure nobody else is
  working on the Master file.}
