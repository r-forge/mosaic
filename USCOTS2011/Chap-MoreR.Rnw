...
For convenience the \verb!mosaic! package provides &read.file()& which uses the file name to
determine which of &read.csv()&, &read.table()&, and \verb!load()! to use and sets the defaults
to 
\begin{itemize}
\item
&header=T&, 
...
@
In fact this is exactly what was done to create the \variable{exper} variable.
<<>>=
...
only certain rows.
\begin{center}
<<faithful-long-xy,fig=true>>=
data(faithful)
names(faithful) <- c('duration', 'time.til.next')
faithfulLong <- subset(faithful, duration > 3)        # any logical can be used to creat subsets
xyplot( time.til.next ~ duration, faithfulLong )
@
\end{center}

Of course, if all we want to do is produce a graph, there is now reason to create 
