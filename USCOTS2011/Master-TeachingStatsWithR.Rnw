...
\node at (0,0) [#3]
{\parbox{1.05in}{\footnotesize {\sc #1 }\texttt{#2}}};
\end{tikzpicture}}
]{%
\parbox{1.2in}{\begin{tikzpicture}
\node at (0,0) [#3]
...
\newcommand{\InstructorNote}[2][For Instructors\\]{%
\tikzNote{#1}{#2}{double copy shadow={opacity=.5},tape,fill=blue!10,draw=blue,thick}
}

\renewcommand{\InstructorNote}[2][For Instructors\\]{%
...
on Teaching Statistics.  Consider them a work in progress.  We hope to continue
...
This material is copyrighted by the authors under a ???? license. 

\textbf{Insert copyright details here.}
...
not work ``as is" in another.  Nevertheless, we'll try to indicate which parts of this 
book fit into each category as we go along.

\subsection*{R and R Packages}

\R\ can be obtained from \url{http://cran.r-project.org/}.  Download and installation are 
pretty straightforward for Mac, PC, or linux machines.
...
The purpose of the MOSAIC community is to help us share ideas and resources to
...
are today usually taught in isolation, if at all.  In particular, we focus on
...
such as the USCOTS 2011 workshop.
